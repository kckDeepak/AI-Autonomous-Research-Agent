...
@pytest.mark.asyncio
async def test_persist_findings_creates_dead_letter_artifact(tmp_path: Path, monkeypatch) -> None:
    ...
    response = await orchestrator.plan_collect_summarize_and_persist_findings(
        PlanRequest(
    ...
    assert response.notion_created_count == 1
    assert response.notion_failed_count == 1
    assert Path(response.notion_persist_artifact_path).exists()
    assert response.notion_dead_letter_artifact_path is not None
    assert Path(response.notion_dead_letter_artifact_path).exists()

    payload = json.loads(Path(response.notion_dead_letter_artifact_path).read_text(encoding="utf-8"))
