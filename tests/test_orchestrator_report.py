...
@pytest.mark.asyncio
async def test_report_stage_writes_report_artifact(tmp_path: Path, monkeypatch) -> None:
    ...
    response = await orchestrator.plan_collect_persist_and_compose_report(
        PlanRequest(
    ...
    assert response.report.references
    assert response.report.citation_index

    payload = json.loads(Path(response.report_artifact_path).read_text(encoding="utf-8"))
